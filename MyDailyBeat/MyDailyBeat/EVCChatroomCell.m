...
- (id) changeChatroom:(MessageChatroom *)chatroom {
    ...
    if ([chatroom.screenNames count] >= 3) {
        ...
        self.chatroomNameLbl.text = [NSString stringWithFormat:@"%@, %@, and %d others", firsts, seconds, [chatroom.screenNames count] - 2];
        
        
        dispatch_queue_t queue = dispatch_queue_create("dispatch_queue_t_dialog", NULL);
        dispatch_async(queue, ^{
            NSURL *imageURL = [[API getInstance] retrieveProfilePictureForUserWithScreenName:firsts];
            NSData *imageData = [NSData dataWithContentsOfURL:imageURL];
            NSURL *imageURL2 = [[API getInstance] retrieveProfilePictureForUserWithScreenName:seconds];
            NSData *imageData2 = [NSData dataWithContentsOfURL:imageURL2];
            NSURL *imageURL3 = [[API getInstance] retrieveProfilePictureForUserWithScreenName:thirds];
